...
class JumpModel:
    ...
    def forward(self, img, is_training, name='coarse'):
        # print(name)
        with tf.name_scope(name):
            with tf.variable_scope(name):
                ...
                out_para = self.make_conv_bn_relu('conv2', out_para, [3, 3, 16, 32], 1, is_training)
                out_para = tf.nn.max_pool(out_para, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')

                out_para = self.make_conv_bn_relu('conv3', out_para, [5, 5, 32, 64], 1, is_training)
                out_para = tf.nn.max_pool(out_para, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')

                out_para = self.make_conv_bn_relu('conv4', out_para, [7, 7, 64, 128], 1, is_training)
                out_para = tf.nn.max_pool(out_para, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')

                out_para = self.make_conv_bn_relu('conv5', out_para, [9, 9, 128, 256], 1, is_training)
                out_para = tf.nn.max_pool(out_para, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')

                out_para = tf.reshape(out_para, [-1, 256 * 20 * 23])
                out_para = self.make_fc('fc1', out_para, [256 * 20 * 23, 256])
                out_para = self.make_fc('fc2', out_para, [256, 2])
...
if __name__ == '__main__':
    model = JumpModel()
    # print(tf.zeros((1, 640, 720, 3)))
    # 第二个参数bool类型化
    out = model.forward(tf.zeros((1, 640, 720, 3)), tf.placeholder(np.bool, name='is_training'))
